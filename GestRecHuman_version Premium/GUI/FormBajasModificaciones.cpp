...
AnsiString cadena = "Dirección particular: ";
//---------------------------------------------------------------------------
...
void __fastcall TFormBajasModific::bitbtnBuscarTrabClick(TObject *Sender)
{
	...
	if(Edit2->Text != "")
	{
		...
		{
			Application->MessageBoxA(LPWSTR("No se obtuvieron resultados en la búsqueda, verifique el número de identidad."),LPWSTR("Error"),MB_OK);
			Edit2->SetFocus();
			return;
		}
		try
		...
		{
			Application->MessageBoxA(LPWSTR("No se ha dado alta al trabajador."),LPWSTR("Información"),MB_OK);
			alta = false;
		}
		if(alta)
		{
...
void __fastcall TFormBajasModific::btnEliminarClick(TObject *Sender)
{
	if(TrabFound)
	{
		if(MessageDlg("¿Está seguro que desea eliminar este trabajador de la empresa?", mtConfirmation, TMsgDlgButtons() << mbYes << mbNo,0) == mrYes)
		{
			empresa->EliminarTrabajador(TrabFound);
			//Application->MessageBoxA("El trabajador fue eliminado con éxito.","Información",MB_OK);
	...
	{
		Application->MessageBoxA("Debe buscar el trabajador que desea eliminar.","Información",MB_OK);
		Edit2->SetFocus();
	} 
}
//---------------------------------------------------------------------------
...
void __fastcall TFormBajasModific::btnAceptarClick(TObject *Sender)
{
 ...
 if((Edit3->Text != "")&&(Edit5->Text != ""))
 {
  ...
  {
   Application->MessageBoxA("Asegúrese de haber indicado almenos, Nombre y Apellidos y Dirección particular.","Información",MB_OK);
   Edit3->SetFocus();
   return;
  }
 btnCancelar1->Click();
